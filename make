...
cd "$des"||exit
curl -fsL https://raw.githubusercontent.com/v2fly/domain-list-community/release/category-ads-all.txt|sed '/^regexp/d'|cut -d: -f2 > reject_v2fly_all
[[ -s reject_v2fly_all ]] || exit 1
...
true > "$des"/reject_v2fly
cd "$src"||exit
for i in $inc
do
  echo "#$i" >> "$des"/reject_v2fly
  sed '/^$/d;/^#/d;/^regexp/d' "$i"|sed 's/^full://g'|sed 's/ @ads//g' >> "$des"/reject_v2fly
  echo "" >> "$des"/reject_v2fly
done

sed -i '/^is.snssdk.com$/d' "$des"/reject_v2fly
